...
def red_monthly_report():
    ...
    """
    df = _load_data()
    month_sum  = df.set_index('order_date').resample('ME').agg(
            order_count=('order_id', 'count'),
            revenue=('amount', 'sum'),
            active_customers=('customer_id', 'nunique')
    )

    month_sum ['avg_order_value'] = month_sum ['revenue'] / month_sum ['order_count']
    
    month_sum ['revenue_growth'] = month_sum ['revenue'].pct_change()
