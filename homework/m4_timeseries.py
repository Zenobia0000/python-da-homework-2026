...
def green_avg_by_month():
    ...
    """
    df = _load_data()
    res = df.groupby(df["order_date"].dt.month)["amount"].mean()
    return res
...
def green_top3_dates():
    ...
    """
    df = _load_data()
    res = df.groupby("order_date")["order_id"].count().sort_values(ascending=False).head(3)
    return res
...
def green_date_range():
    ...
    """
    df = _load_data()
    min = df["order_date"].min()
    max = df["order_date"].max()
    return ((min, max))
...
def yellow_rolling_avg(monthly_revenue):
    ...
    """
    res = monthly_revenue.rolling(window=3).mean()
    return res
...
def yellow_category_median(df):
    ...
    """
    res = df.groupby("category")["amount"].median().sort_values(ascending=False)
    return res
...
def red_monthly_report():
    ...
    df = _load_data()
    ts = df.set_index("order_date").sort_index()
    res = ts.resample("ME").agg(
    order_count = ("order_id", "count"),
    revenue = ("amount", "sum"), 
    active_customers = ("customer_id", "nunique")
    )
    res["avg_order_value"] = res["revenue"] / res["order_count"]
    res["revenue_growth"] = res["revenue"].pct_change()
    return res