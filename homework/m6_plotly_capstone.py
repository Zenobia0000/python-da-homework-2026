...
def green_plotly_line():
    ...
    df["year_mon"] = df["order_date"].dt.to_period("M").astype(str)

    monthly = df.groupby("year_mon", as_index=False)["amount"].sum()

    fig = px.line(
        monthly, x="year_mon", y="amount", markers=True, title="Monthly Revenue Trend"
    )
...
def green_plotly_pie():
    ...
    vip = df["vip_level"].value_counts().reset_index()
    vip.columns = ["vip_level", "count"]

    fig = px.pie(vip, names="vip_level", values="count", title="VIP Distribution")
